...
3. Successful requests either set auth cookies (`mode=cookie`) or return a one-time token that redirects through `/wp-login.php?action=gn_token_login` to establish cookies in the browser.
