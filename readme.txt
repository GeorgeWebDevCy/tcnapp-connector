...
* `/wp-json/gn/v1/login` – POST authentication requiring a matching username or email address alongside the password, issuing seven-day token hand-offs plus rate limiting and optional device locking filters.
...
= 0.3.100 =
* Harden the login and API token filters so URL-like values supplied via `gn_password_api_issue_login_token` or `gn_password_api_issue_api_token` are discarded in favour of random opaque tokens, preventing callbacks from hijacking the token with redirects.
...
= 0.3.93 =
* Allow `/wp-json/gn/v1/login` and the `/jwt-auth/v1/token` compatibility route to accept either a username or email alongside the password so clients can authenticate with whichever identifier they supply while keeping credential validation aligned with WordPress.

= 0.3.92 =
* Bundle the `TCN\\Platform\\Support\\ErrorCodes` helper so authentication endpoints can build WP_Error responses instead of crashing when Composer autoloading is unavailable.
...
= 0.3.88 =
* Prepare plugin metadata for the 0.3.88 release.
...
* Reuse cached avatar URL metadata across contexts and respect requested schemes when WordPress renders avatars.

= 0.3.79 =
* Add presets for the `/gn/v1/discounts/lookup`, `/discounts/transactions`, and `/discounts/history` endpoints to the API tester so administrators can validate the QR redemption flow end-to-end.

= 0.3.78 =
* Fix the API tester avatar upload example so the preset renders without PHP warnings or fatal errors.
