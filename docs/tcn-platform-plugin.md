...
- **JWT Auth Secret** – Define `JWT_AUTH_SECRET_KEY` in `wp-config.php` with a long random string so the `/jwt-auth/v1` compatibility endpoints continue issuing signed tokens. Password Login API responses always return opaque bearer strings.
...
- Refresh the platform reference manual to document the discount API surface alongside existing authentication, membership, and WooCommerce routes.
