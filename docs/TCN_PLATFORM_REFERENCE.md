...
| ----- | ------ | ---- | ------- |
| `/login` | POST | Optional (`mode=token` default; `mode=cookie` requires credentials) | Authenticate by username/email + password. Prefers seven-day token hand-offs but can still issue cookies for same-origin flows. |
...
- `mode`: Defaults to `token`, issuing a seven-day hand-off token for `/wp-login.php?action=gn_token_login`. Set to `cookie` to receive immediate WordPress auth cookies instead.
