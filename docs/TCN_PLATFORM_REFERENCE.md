...
| ----- | ------ | ---- | ------- |
| `/login` | POST | None | Authenticate by username/email + password. Always returns a seven-day token hand-off for `/wp-login.php?action=gn_token_login`. |
...
| `/change-password` | POST | Logged-in user (cookie, `X-WP-Nonce`, or Bearer token) | Change the current user password after verifying the existing password. |
...
- `current_password` and `password`: Required on `/change-password`; both values must be present and the current password must match the stored hash.
...
- Success payloads contain `success: true`, along with contextual data such as the one-time `token` (valid for seven days), `api_token`, and the authenticated `user` profile.
