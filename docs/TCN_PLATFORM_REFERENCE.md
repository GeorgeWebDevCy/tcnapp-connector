...
* **Token filters:** Filters `gn_password_api_issue_login_token` and `gn_password_api_issue_api_token` can override the generated login and bearer strings before they are cached. Returning an empty string or a URL-like value falls back to opaque random tokens so redirects or absolute URLs supplied via the filters are ignored.【F:includes/Auth/PasswordLoginService.php†L964-L1037】
